...
class RtmpProxy:
    ...
    def set_logger(self):
        format = '[%(asctime)-15s] [%(levelname)s] [%(filename)s:%(lineno)d] [%(thread)d] %(message)s'
        log_level = self.get_loglevel()
        logging.basicConfig(filename='rtmp_proxy.log', level=log_level, format=format)
    ...
    def do_publish(self, pull_url):
        ...
        thread.start_new_thread(self.run_ffmpeg, (channel_id, pull_url, push_url));

    def run_ffmpeg(self, channel_id, pull_url, push_url):
        start_time = int(time.time()) - 60;
        publish_cmd = 'ffmpeg -i %s -c copy -bsf:a aac_adtstoasc -f flv "%s"' % (pull_url, push_url)
        logging.info("publish cmd is %s" % publish_cmd)
        (ret, infolog, errlog) = runcmd(publish_cmd, True)#not logging.isEnabledFor(logging.DEBUG))
        if ret > 0:
            logging.error("ffmpeg log:\n%s" % infolog)
            logging.error("ffmpeg log:\n%s" % errlog)
            logging.error("publish ret: %d cmd %s" % (ret, publish_cmd))
        else :
            logging.debug("ffmpeg log:\n%s" % infolog)
            logging.info("publish ret: %d cmd %s" % (ret, publish_cmd))
